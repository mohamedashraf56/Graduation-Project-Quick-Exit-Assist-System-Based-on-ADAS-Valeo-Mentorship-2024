...
class RadialBar : public QQuickPaintedItem   //a RadialBar class that inherits from QQuickPaintedItem
{
    Q_OBJECT //macro is necessary for using signals and slots in the class. It also enables the use of other Qt-specific features like property bindings


    // class properties using Q_PROPERTY These properties can be manipulated from QML and are accompanied by getter and setter functions. Properties include size, angles, colors, font, etc.
...
public:
    RadialBar(QQuickItem *parent = 0);  //Constructor for the RadialBar class.
    void paint(QPainter *painter);       //Public Functions to customize the rendering behavior.
...
private:
    //private members variables used to store the state of the radial bar.
    qreal m_Size;
    qreal m_StartAngle;
    qreal m_SpanAngle;
    qreal m_MinValue;
