...
sint8 I2C_Init(I2C_Config_St * Config_St)
{
	...
	{
		// enable the clock on the periphral
		RCC_Enable_Clock(RCC_APB1, RCC_I2C2);
		// enable event interrupt
		NVIC_En_Interrupt(NVIC_I2C2_EV);
		// enable error interrupt
		NVIC_En_Interrupt(NVIC_I2C2_ER);
		// Disable the periphral
		CLEAR_BIT(I2C_2->I2C_CR1,0);
		// select periphral freq on APB1 BUS
		// IN This case the freq i will choos 20Mhz
		INSERST_BITS(I2C_2->I2C_CR2,0,6,20);
		// Disable event and error interrupts
		//SET_BIT(I2C_1->I2C_CR2,8);
		//SET_BIT(I2C_1->I2C_CR2,9);
		// choose the fast mode in CCR REGISTER
		SET_BIT(I2C_2->I2C_CCR,15);
		// make duty = 1 to reach 400khz
		SET_BIT(I2C_2->I2C_CCR,14);
		// CCR CALULATION TO CONFIGURE THE CLOCK WITH 400K AND SYSCLCOK IS 20MHZ
		// CCR = 2 ;
		INSERST_BITS(I2C_2->I2C_CCR,0,12,2);
		// CONFIGURE THE Trise TIME with 400khz clock i2c and 20mhz Freq i2c
		// in this case TRISE IS EQUAL 7
		INSERST_BITS(I2C_2->I2C_TRISE,0,6,7);
		// CLOCK STRETCHING ENABLE
		CLEAR_BIT(I2C_2->I2C_CR1,7);
		// GENERAL CALL DISABLED
		CLEAR_BIT(I2C_2->I2C_CR1,6);


		SET_BIT(I2C_2->I2C_CR1,0);
	}
	break;
	case 3 :
	{
		// enable the clock on the periphral
		RCC_Enable_Clock(RCC_APB1, RCC_I2C3);
		// enable event interrupt
		NVIC_En_Interrupt(NVIC_I2C3_EV);
		// enable error interrupt
		NVIC_En_Interrupt(NVIC_I2C3_ER);
		// Disable the periphral
		CLEAR_BIT(I2C_3->I2C_CR1,0);
		// select periphral freq on APB1 BUS
		// IN This case the freq i will choos 20Mhz
		INSERST_BITS(I2C_3->I2C_CR2,0,6,20);
		// Disable event and error interrupts
		//SET_BIT(I2C_1->I2C_CR2,8);
		//SET_BIT(I2C_1->I2C_CR2,9);
		// choose the fast mode in CCR REGISTER
		SET_BIT(I2C_3->I2C_CCR,15);
		// make duty = 1 to reach 400khz
		SET_BIT(I2C_3->I2C_CCR,14);
		// CCR CALULATION TO CONFIGURE THE CLOCK WITH 400K AND SYSCLCOK IS 20MHZ
		// CCR = 2 ;
		INSERST_BITS(I2C_3->I2C_CCR,0,12,2);
		// CONFIGURE THE Trise TIME with 400khz clock i2c and 20mhz Freq i2c
		// in this case TRISE IS EQUAL 7
		INSERST_BITS(I2C_3->I2C_TRISE,0,6,7);
		// CLOCK STRETCHING ENABLE
		CLEAR_BIT(I2C_3->I2C_CR1,7);
		// GENERAL CALL DISABLED
		CLEAR_BIT(I2C_3->I2C_CR1,6);




		SET_BIT(I2C_3->I2C_CR1,0);
...
sint8 I2C_Gpio_Config(I2C_Config_St * Config_St)
{
	...
	case 1 : // PINS PB6 SCL1   PB7 SDA1
	{
		...
		/*
		GPIO_Set_Pin_Direction(PORTB, PIN8, GPIO_MODE_ALTFUN);
		GPIO_Set_Pin_Direction(PORTB, PIN9, GPIO_MODE_ALTFUN);

		GPIO_Alternate_Function(PORTB, PIN8, (0b0100));
		GPIO_Alternate_Function(PORTB, PIN9, (0b0100));

		// SELECT THE OPEN DRAIN
		GPIO_Set_Output_Type(PORTB, PIN8, OPEN_DRAIN);
		GPIO_Set_Output_Type(PORTB, PIN9, OPEN_DRAIN);

		// select the output speed very high
		GPIO_Set_Output_Speed(PORTB, PIN8, VeryHighSpeed);
		GPIO_Set_Output_Speed(PORTB, PIN9, VeryHighSpeed);
	...
	case 2 :
	{
		...
		GPIO_Set_Pin_Direction(PORTB, PIN10, GPIO_MODE_ALTFUN);
		GPIO_Set_Pin_Direction(PORTB, PIN11, GPIO_MODE_ALTFUN);

		GPIO_Alternate_Function(PORTB, PIN10, (0b0100));
		GPIO_Alternate_Function(PORTB, PIN11, (0b0100));

		// SELECT THE OPEN DRAIN
		GPIO_Set_Output_Type(PORTB, PIN10, OPEN_DRAIN);
		GPIO_Set_Output_Type(PORTB, PIN11, OPEN_DRAIN);

		// select the output speed very high
		GPIO_Set_Output_Speed(PORTB, PIN10, VeryHighSpeed);
		GPIO_Set_Output_Speed(PORTB, PIN11, VeryHighSpeed);
	...
	case 3 : // NEED EDIT
	{
		...
		// ENABLE CLOCK ON GPIOA
			...
			GPIO_Set_Pin_Direction(PORTB, PIN8, GPIO_MODE_ALTFUN);
			GPIO_Set_Pin_Direction(PORTB, PIN4, GPIO_MODE_ALTFUN);

			// SET AF FUNCTION TO IC2
			GPIO_Alternate_Function(PORTB, PIN8, (0b0100));
			GPIO_Alternate_Function(PORTB, PIN4, (0b0100));

			// SELECT THE OPEN DRAIN
			GPIO_Set_Output_Type(PORTB, PIN8, OPEN_DRAIN);
			GPIO_Set_Output_Type(PORTB, PIN4, OPEN_DRAIN);

			// select the output speed very high
			GPIO_Set_Output_Speed(PORTB, PIN8, VeryHighSpeed);
			GPIO_Set_Output_Speed(PORTB, PIN4, VeryHighSpeed);
...
uint8 I2C_Read_Byte(I2C_Config_St * Config_St,uint8 SLAVE_Address,uint8 REGISTER_Address)
{
	...
	case 2 :
	{
			...
			while(BIT_IS_SET(I2C_2->I2C_SR2,1))
			{

		    }
			// Generate a start condition
			SET_BIT(I2C_2->I2C_CR1,8);
			// wait untill start condition is generated
			// clear by reading
			while(BIT_IS_CLEAR(I2C_2->I2C_SR1,0))
				{
			...
			// send slave address + write
			I2C_2->I2C_DR = (   (SLAVE_Address << 1)  |  (0)   );

			// WAIT untill address matched flag is 1
			while(BIT_IS_CLEAR(I2C_2->I2C_SR1,1))
			{
			...
			(void)I2C_2->I2C_SR2;
			// Send register address
			I2C_2->I2C_DR = REGISTER_Address ;
			// wait untill Byte transfer finished
			while(BIT_IS_CLEAR(I2C_2->I2C_SR1,2))
			{
			...
			I2C_2->I2C_DR = (   (SLAVE_Address << 1)  |  (1) );
			// WAIT untill address matched flag is 1
			while(BIT_IS_CLEAR(I2C_2->I2C_SR1,1))
			{
			...
			(void)I2C_2->I2C_SR2;
			// Disable ACK
			CLEAR_BIT(I2C_2->I2C_CR1,10);
			// WAIT TO READ DATA
			while(BIT_IS_CLEAR(I2C_2->I2C_SR1,6))
			{

			}

			RECEIVED_Data = I2C_2->I2C_DR;

			// Generate a stop condition
			SET_BIT(I2C_2->I2C_CR1,9);
	...
	{
		// check if the bus is busy
		while(BIT_IS_SET(I2C_3->I2C_SR2,1))
		{

	    }

		// Generate a start condition
		SET_BIT(I2C_3->I2C_CR1,8);
		// wait untill start condition is generated
		// clear by reading
		while(BIT_IS_CLEAR(I2C_3->I2C_SR1,0))
			{
		...
		// send slave address + write
		I2C_3->I2C_DR = (   (SLAVE_Address << 1)  |  (0)   );

		// WAIT untill address matched flag is 1
		while(BIT_IS_CLEAR(I2C_3->I2C_SR1,1))
		{
		...
		(void)I2C_3->I2C_SR2;
		// Send register address
		I2C_3->I2C_DR = REGISTER_Address ;
		// wait untill Byte transfer finished
		while(BIT_IS_CLEAR(I2C_3->I2C_SR1,2))
		{
		...
		I2C_3->I2C_DR = (   (SLAVE_Address << 1)  |  (1) );
		// WAIT untill address matched flag is 1
		while(BIT_IS_CLEAR(I2C_3->I2C_SR1,1))
		{
		...
		(void)I2C_3->I2C_SR2;
		// Disable ACK
		CLEAR_BIT(I2C_3->I2C_CR1,10);
		// WAIT TO READ DATA
		while(BIT_IS_CLEAR(I2C_3->I2C_SR1,6))
		{

		}

		RECEIVED_Data = I2C_3->I2C_DR;

		// Generate a stop condition
		SET_BIT(I2C_3->I2C_CR1,9);
	}
	break;
	default :
		return 0;
...
uint8 I2C_Write_Byte(I2C_Config_St * Config_St,uint8 SLAVE_Address,uint8 REGISTER_Address, uint8 Data)
{
	...
	case 2 :
	{
		...
		I2C_2->I2C_DR = REGISTER_Address ;
		// wait untill Byte transfer finished
		while(BIT_IS_CLEAR(I2C_2->I2C_SR1,7))
		{
	...
	case 3 :
	{
		...
		I2C_3->I2C_DR = REGISTER_Address ;
		// wait untill Byte transfer finished
		while(BIT_IS_CLEAR(I2C_3->I2C_SR1,7))
		{
